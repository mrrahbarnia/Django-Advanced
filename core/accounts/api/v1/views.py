...
class RegistrationAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        message = EmailMessage('email/activation-email.tpl', {'token':token}, 'admin@admin.com',to=[email])
...
class EmailActivationResendAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user_object)
        message = EmailMessage('email/activation-email.tpl', {'token':token}, 'admin@admin.com',to=[user_object.email])
        EmailThread(message).start()
        return Response({"detail":"The verificatione email resent successfully"},status=status.HTTP_200_OK)
...
class RePasswordEmailRequestApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        EmailThread(message).start()
        return Response({
            "detail":"The email has been sent for you."
            })
...
class RePasswordValidateTokenApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"detail":"Token is not valid"},status=status.HTTP_400_BAD_REQUEST)
        return Response({"detail":"The token is valid"},status=status.HTTP_200_OK)
