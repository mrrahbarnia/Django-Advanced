    ...
    def validate(self, attrs):
        ...
        if username and password:
            ...
            if not user.is_verified:
                raise serializers.ValidationError({"detail":"The user is not verified"})
        else:
...
class EmailActivationResendSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if user_object.is_verified:
            return serializers.ValidationError({"detail":"user has already been verified and activated. "})
...
class SetNewPasswordSerializer(serializers.Serializer):
    ...
    password1 = serializers.CharField(min_length=8,max_length=20,required=True)
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            raise serializers.ValidationError({'password':list(e.messages)})
        try:
            token = jwt.decode(attrs.get('token'),
